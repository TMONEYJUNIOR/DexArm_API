...
def sway_arm(x_start, x_end, y, z, repetitions=5, delay=0.5):
    #Moves the DexArm back and forth on the X-axis.
    for _ in range(repetitions):
        dexarm.move_to(x_start, y, z)  # Move left
        time.sleep(delay)
        dexarm.move_to(x_end, y, z)  # Move right
        time.sleep(delay)
...
def cycle_lights(canvas, red_light, yellow_light, green_light):
    ...
    def update_lights():
        ...
        if colors[current_color] == "red":
            ...
            sway_arm(100, 200, 50, 150)  # Sway from X=100 to X=200 at Y=50, Z=150
